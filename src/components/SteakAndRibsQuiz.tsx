...
export function SteakAndRibsQuiz() {
  ...
  const generateQuiz = () => {
    let shuffledSteakAndRibs: SteakRibsDish[] = shuffleArray(steakAndRibsDishes);

    while (shuffledSteakAndRibs.length < 8) {
      shuffledSteakAndRibs = shuffledSteakAndRibs.concat(shuffleArray(steakAndRibsDishes));
    }

    // ✅ Description-Based Questions (Shows only text, asks for item name)
    const descriptionQuestions = shuffledSteakAndRibs.slice(0, 3).map((dish) => ({
    ...
    const priceQuestions = shuffledSteakAndRibs.slice(3, 5).map((dish) => ({
    ...
    const imageQuestions = shuffledSteakAndRibs.slice(5, 8).map((dish) => ({
