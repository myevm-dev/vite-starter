...
import { sandwichesAndBurgers, SandwichBurger } from "../data/food/sandwichesAndBurgers";
...
export function SandwichesAndBurgersQuiz() {
  ...
  const generateQuiz = () => {
    let shuffledItems: SandwichBurger[] = shuffleArray(sandwichesAndBurgers);

    while (shuffledItems.length < 8) {
      shuffledItems = shuffledItems.concat(shuffleArray(sandwichesAndBurgers));
    }

    // ✅ Description-Based Questions (Shows only text, asks for item name)
    const descriptionQuestions = shuffledItems.slice(0, 3).map((dish) => ({
    ...
    const priceQuestions = shuffledItems.slice(3, 5).map((dish) => ({
    ...
    const imageQuestions = shuffledItems.slice(5, 8).map((dish) => ({
